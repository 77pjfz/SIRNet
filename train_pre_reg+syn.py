...
import losses
import re
from Networks import pre_reg_syn_net
...
def train(args):
    ...
    model = pre_reg_syn_net.Net(for_train=True,use_checkpoint=True)
    model.to(device)
    ...
    train_files_mov = glob.glob(os.path.join(args.train_dir_mov,'train', '*.nii'))
    train_files_fix = glob.glob(os.path.join(args.train_dir_fix,'train', 'image1', '*.nii.gz'))
    ...
    for i in range (311,args.epoch + 1 ):
        for data in DL:
            ...
            sys_loss_normal =  Mse_loss(sysimg*A,warped*A)
            
            #总loss
            loss = sim_loss + grad_loss  + sys_loss_normal
            
            print("i: %d fix: %s mov: %s mask: %s loss: %f grad_loss:%f sim_loss:%f sys_loss_normal:%f"
                  % (i, fix_name,mov_name,mask_name, loss.item(), grad_loss.item(),sim_loss.item(),sys_loss_normal.item()),flush=True)                                                                                                         
            print("%d, %s, %s, %s %f, %f, %f, %f"
                  % (i, fix_name,mov_name,mask_name, loss.item(), grad_loss.item(),sim_loss.item(),sys_loss_normal.item()),file=f)
