...
class Net(nn.Module):
    ...
    def forward(self,moving,fixed,atlas,mask,A,S):        
        with torch.no_grad():     
            fix_enc_P = self.Encoder(fixed)
            atlas_enc = self.Encoder(atlas)
            
            sysimg_P,fsys_feat_P = self.Mor_Decoder(fix_enc_P)
            flow_P = self.Reg_Decoder(atlas_enc,fix_enc_P,fsys_feat_P)
        
        warpedP = self.STN(atlas, flow_P)
        F=torch.mean(fixed)
        P=torch.mean(atlas)
        k = F/P
        ...
        fix_enc_FP = self.Encoder(FP)
        fix_enc = self.Encoder(fixed)
        mov_enc = self.Encoder(moving)  
       
        # with torch.no_grad(): 
        sysimg,fsys_feat = self.Mor_Decoder(fix_enc_FP) #sysimg相当于F~
        
        flow = self.Reg_Decoder(mov_enc,fix_enc,fsys_feat)
    
        if self.for_train:
            warped1 = self.STN(moving, flow) #相当于M(Φ)
            
            
            return warped1,sysimg,flow,FP,warpedP,FPA,FPS

        
        else:
            warped1 = self.STN(moving, flow)
...
class Morphology_decoder(nn.Module):
    def __init__(self, 
        ...
        self.conv_out = nn.Conv3d(channel_num*2,1,kernel_size=3,stride=1,padding='same')  
    ...
    def forward(self,x_enc):
        ...
        pred_img = self.conv_out(y4)
        
        feat = [y1, y2, y3, y4]
        return pred_img,feat
...
class Registration_decoder(nn.Module):
    ...
    def forward(self,m_enc,f_enc,fsys_feat):
        ...
        y4 = self.att4(y,x_fix_4,x_mov_4,fsys_feat[0])    # 64,64.64.64,64
        y4 = self.conv_4(y4)
        ...
        y3  = self.att3(y,x_fix_3,x_mov_3,fsys_feat[1])
        y3 = self.conv_3(y3)
        ...
        y2 = self.att2(y,x_fix_2,x_mov_2,fsys_feat[2])
        y2 = self.conv_2(y2)
        ...
        y1 = self.att1(y,x_fix_1,x_mov_1,fsys_feat[3])
        y1 = self.conv_1(y1)
...
class att_registration(nn.Module):
    ...
    def fuse_forward(self,Fuse,R_f,R_m,S):   
        ...
        return out
    def forward(self,Fuse,R_f,R_m,S):
        if self.use_checkpoint and Fuse.requires_grad:
            x_out = checkpoint.checkpoint(self.fuse_forward, Fuse,R_f,R_m,S)
        else:
            x_out = self.fuse_forward(Fuse,R_f,R_m,S)
        return x_out
